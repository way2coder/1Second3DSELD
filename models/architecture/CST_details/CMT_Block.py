...
from models.architecture.CST_details.CST_encoder import CST_attention
from .layers import LocalPerceptionUint, InvertedResidualFeedForward, Temp_attention, Spec_attention, ScConv, SRUCRU
...
class CMT_Layers(torch.nn.Module):
    def __init__(self, params, temp_embed_dim, ffn_ratio=4., drop_path_rate=0.):
        ...
        if self.conv_type == 'scconv':
            self.ScConv = ScConv(self.dim)
            self.SRUCRU = SRUCRU(int(self.dim))
        elif self.conv_type == 'origin':
            self.LPU = LocalPerceptionUint(self.dim)
            self.IRFFN = InvertedResidualFeedForward(self.dim, self.ffn_ratio)
    ...
    def forward(self, x):
        if not self.cmt_split:
            if not self.ch_attn_dca: # ch_attn (ULE) or dst_attn  this way 
                if self.conv_type == 'scconv':
                    scconv = self.ScConv(x)
                    x = x + scconv
                elif self.conv_type == 'srucru':
